...
using IoTControlTower.Application.Devices.Commands.Notifications;

namespace IoTControlTower.Application.Devices.Commands;

public class CreateDeviceCommand : DeviceCommandBase
{
    public class CreateDeviceCommandHandler(UnitOfWork unitOfWork, IValidator<CreateDeviceCommand> validator, IMediator mediator, ILogger<CreateDeviceCommandHandler> logger) : IRequestHandler<CreateDeviceCommand, Device>
    {
        ...
        public async Task<Device> Handle(CreateDeviceCommand request, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogInformation("CreateDeviceCommand", request.Description);
